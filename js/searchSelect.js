(function(){
	...
	SearchSelect.prototype={
		...
		draw:function(){
			...
			this.selectObject.unshift({title:this.panel,container:this.selectContainer});
		...
		checkAll :function(bool){
			...
			for(var i=0,_len=liList.length;i<_len;i++){
				...
				if(bool){
					this.checkList[_val]={value:_val,name:_name};
        ...
        getCheckedObj:function(){
            ...
            var arr=[];
            for(var item in checkList){
                arr.push({name:checkList[item].name,value:checkList[item].value});
            }
            return arr;            
        },
