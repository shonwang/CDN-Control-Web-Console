    ...
    var IpManageView = Backbone.View.extend({
        ...
        updateIpTable: function(){
            ...
            _.each(this.ipList, function(el, index, list){
                ...
                if (el.status == 4 || el.status == 20 || el.status == 28 ) el.statusName = "<span class='text-danger'>宕机</span>";
                if (el.status == 2 || el.status == 8 || el.status == 10 || el.status == 16 || el.status == 18 || el.status == 24 || el.status == 26) el.statusName = "<span class='text-warning'>暂停中</span>";
                if (el.status == 6 || el.status == 12 || el.status == 14 || el.status == 22 || el.status == 30) el.statusName = "暂停且宕机";
            }.bind(this))
