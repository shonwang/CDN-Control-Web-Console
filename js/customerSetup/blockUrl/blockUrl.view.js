define('blockUrl.view', ['utility', 'template'], function(Utility, template) {
  var DetailView = Backbone.View.extend({
    ...
    initTable: function(res) {
      ...
      this.table = $(_.template(template['tpl/customerSetup/blockUrl/TabCurrentBlockDetailList.table.html'])({
        data: _data
      }));
  ...
  var TabBlockUrlView = Backbone.View.extend({
    ...
    initialize: function(options) {
      ...
      this.collection.getGuestQuotaCount({
  ...
  var TabCurrentBlockListView = Backbone.View.extend({
    ...
    initblockListDropmenu: function() {
      ...
      var pageNum = [{
          ...
          name: this.numberControl + "条",
          value: this.numberControl
        }
        //{name: "100条", value: 100}
      ];
    ...
    initTable: function() {
      ...
      this.table = $(_.template(template['tpl/customerSetup/blockUrl/TabCurrentBlockList.table.html'])({
        data: data
      }));
  ...
  var TabHistoryView = Backbone.View.extend({
    ...
    initTable: function() {
      var data = this.collection.models;
      this.table = $(_.template(template['tpl/customerSetup/blockUrl/TabHistory.table.html'])({
        data: data
      }));
      if (data.length == 0) {
        this.setNoData("未查到符合条件的数据，请重新查询");
  ...
  var BlockUrlView = Backbone.View.extend({
    ...
    onShownTab: function(event) {
      ...
      switch (id) {
        case '#blockUrl':
          if (this.myTabBlockView) this.myTabBlockView.collection.getGuestQuotaCount({
            userId: this.userInfo.uid
          });
          break;
        case '#blockUrlList':
